...
namespace Intersect.Server.Database.PlayerData.Migrations
{
    public partial class GuildBankMaxSlotMigration
    {
        ...
        {
            Log.Information("Checking to see if there are any guilds exceeding the configured max bank size...");
            ...
            foreach (var guild in context.Guilds)
            {
                ...
                {
                    Log.Information($"Too many bank slots ({guild.BankSlotsCount}) for guild {guild.Name}. Setting to {Options.Instance.Bank.MaxSlots}.");
