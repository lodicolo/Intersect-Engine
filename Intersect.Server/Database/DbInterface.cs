...
namespace Intersect.Server.Database
{
    public static class DbInterface
    {
            ...
            lock (mGameDbLock)
            {
                switch (gameObjectType)
                {
                    ...
                    case GameObjectType.Group:
                        foreach (var group in sGameDb.GroupTypes)
                        {
                            GroupTypeDescriptor.Lookup.Set(group.Id, group);
                        }
        ...
        public static IDatabaseObject AddGameObject(GameObjectType gameObjectType) =>
            AddGameObject(gameObjectType, Guid.Empty);
        ...
        {
            if (GroupTypeDescriptor.Lookup.Count > 0)
            {
                return;
            }

            AddGameObject(GameObjectType.Group, GroupTypeDescriptor.CreatePartyDescriptor());
        }
        ...
        public static object GetPlayerContextLock() => mPlayerDbLock;

        public static PlayerContext GetPlayerContext() => sPlayerDb;
