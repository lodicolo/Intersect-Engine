...
[Route("logs")]
[Authorize]
public sealed class LogsController : IntersectControllerBase
{
    [Route("chat")]
    [HttpGet("chat")]
    ...
    )
    {
        ...
        using (var context = DbInterface.LoggingContext)
        {
            ...
            messages = messages.Where(
                m => (m.PlayerId == player1Id && m.TargetId == player2Id) ||
                     (m.PlayerId == player2Id && m.TargetId == player1Id)
            );
    ...
    )
    {
        ...
        using (var context = DbInterface.LoggingContext)
        {
            ...
            var ipAddresses = history
                .Where(m => m.UserId == userId && m.UserId != Guid.Empty && !string.IsNullOrWhiteSpace(m.Ip))
                ...
                .Select(
                    ...
                    {
                        Ip = m.First().Ip,
                        LastUsed = m.First().TimeStamp
                    }
                );
            var addresses = ipAddresses.Skip(page * pageSize).Take(pageSize).ToList();

            //Foreach IP Address Find Other Users
            using (var ctx = DbInterface.CreatePlayerContext())
            {
                foreach (var addr in addresses)
                {
                    var otherUsers = context.UserActivityHistory
                        .Where(m => m.Ip == addr.Ip && m.UserId != userId && !string.IsNullOrWhiteSpace(m.Ip))
                        .GroupBy(m => m.UserId)
                        .Select(m => m.First().UserId)
                        .ToList();
    ...
    )
    {
        ...
        using (var context = DbInterface.CreateLoggingContext())
        {
            ...
            if (doGroupBy)
            {
                trades = trades.GroupBy(t => t.TradeId, (key, group) => group.OrderBy(gt => gt.PlayerId))
                    .Select(t => t.First());
            }
    ...
    {
        var userIds = guildActivity.Where(m => m.UserId != Guid.Empty)
            .GroupBy(m => m.UserId)
            .Select(m => m.First().UserId)
            .ToList();
        var playerIds = guildActivity.Where(m => m.PlayerId != Guid.Empty)
            .GroupBy(m => m.PlayerId)
            .Select(m => m.First().PlayerId)
            .ToList();
        var targetIds = guildActivity.Where(m => m.InitiatorId != Guid.Empty)
