...
public abstract class LocaleCommandNamespace : LocaleNamespace
{
    ...
    {
        var commands = GetType().GetMembers(
        ...
        CommandLookup = commands.Select(
            command =>
            {
                ...
                return new KeyValuePair<string, LocaleCommand>(
                    command.Name,
                    command
                );
            }
    ...
    [JsonIgnore] public ImmutableList<LocaleCommand> CommandList { get; }

    [JsonIgnore] public IDictionary<string, LocaleCommand> CommandLookup { get; }
}
