...
public static class LocaleDictionaryTypeExtensions
{
    ...
    {
        var localeDictionaryParameters = type.FindGenericTypeParameters(
            typeof(LocaleDictionary<,>)
        );
        return new(
            localeDictionaryParameters[0],
            localeDictionaryParameters[1]
        );
    }

    public static bool IsLocaleDictionary(this Type type) => type.Extends(
        typeof(LocaleDictionary<,>)
    );
