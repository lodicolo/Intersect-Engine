...
public class LocaleDictionary<TKey, TValue> : Localized, IDictionary<TKey, TValue> where TValue : Localized
{
    ...
    public TValue this[TKey key]
    {
        ...
        set
        {
            var target = _frozen || _defaults.ContainsKey(
                key
            )
                ? _overrides
                : _defaults;
    ...
    public ICollection<TValue> Values =>
        Keys.Select(
            key => this[key ?? throw new InvalidOperationException()]
        ).ToList();
    ...
    {
        var source = _frozen || ContainsKey(
            key
        )
            ? _overrides
            : _defaults;
    ...
    public bool ContainsKey(TKey key) => _defaults.ContainsKey(
        key
    );
    ...
    public bool Remove(TKey key) => (_frozen || _defaults.Remove(
        key
    )) && _overrides.Remove(
        key
    );
    ...
    public void Add(KeyValuePair<TKey, TValue> item) => Add(
        item.Key,
        item.Value
    );
    ...
    public bool Remove(KeyValuePair<TKey, TValue> item) => (_frozen || _defaults.Remove(
        item
    )) && _overrides.Remove(
        item
    );
