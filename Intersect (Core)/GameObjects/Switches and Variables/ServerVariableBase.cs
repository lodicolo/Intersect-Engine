...
public class ServerVariableBase : DatabaseObject<ServerVariableBase>, IFolderable
{
    ...
    [NotMapped] [JsonIgnore] public VariableValue Value { get; set; } = new();
    ...
    public static string[] GetNamesByType(VariableDataType dataType) => Lookup
    ...
    public static int ListIndex(Guid id, VariableDataType dataType) => Lookup
        .Where(pair => pair.Value is ServerVariableBase descriptor && descriptor.Type == dataType)
        .OrderBy(pair => pair.Value.TimeCreated)
        .ToList()
    ...
    public static Guid IdFromList(int listIndex, VariableDataType dataType)
    {
        ...
        return Lookup.Where(pair => pair.Value is ServerVariableBase descriptor && descriptor.Type == dataType)
            .OrderBy(pair => pair.Value.TimeCreated)
            .Skip(listIndex)
            .First()
            .Value.Id;
