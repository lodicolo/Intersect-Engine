...
public sealed class CommandParser
{
    ...
    {
        Settings = settings;
        Tokenizer = new(settings.TokenizerSettings);
    ...
    private bool Register(ICommand command)
    {
    ...
    public ICommand? Find(string commandName) => !Lookup.TryGetValue(
        commandName,
        out var command
    ) ? default : command;
    ...
    {
        var tokens = Tokenizer.Tokenize(line).Select(token => token.Trim()).ToList();
        ...
        if (command == null)
        {
            return MissingCommandError.Create(
                commandName,
                Settings.Localization.Errors.CommandNotFound,
                Settings.Localization.HelpCommand
            ).AsResult();
        }
        ...
        for (var tokenIndex = 1; tokenIndex < tokens.Count; ++tokenIndex)
        {
            ...
            if (!canBeShortName && !canBeLongName)
            {
                ...
                {
                    errors.Add(new(Settings.Localization.Errors.BadArgumentFormat.ToString(token)));
            ...
            if (Settings.Localization.TypeNames.TryGetValue(
                    typeName,
                ...
                ))
            {
                if (!argument.AllowsMultiple)
                {
                    errors.Add(
                        new(
                            Settings.Localization.Errors.DuplicateNamedArgument.ToString(argumentDisplayName),
                            false
                        )
                    );
            ...
            else if (argument.IsCollection)
            {
                ...
                else
                {
                    ...
                    var parsedPartValues = cleanArgValue.Split(
                        (string[]?)new[] { argument.Delimeter },
        ...
        foreach (var argument in command.Arguments)
        {
            if (!argument.IsRequired(parserContext) || parsed.ContainsKey(argument))
            {
                ...
                {
                    parsed[argument] = ConstructDefaultArgument(
                        argument,
                        true
                    );
    ...
    private ArgumentValuesMap ConstructDefaultArguments(IEnumerable<ICommandArgument> arguments) => new(
        arguments.Where(argument => argument != null).Select(
    ...
    private ArgumentValues ConstructDefaultArgument(
        ICommandArgument argument,
        bool isImplicit = false
    )
    {
        var argumentName = argument.ShortName == '\0' ? Settings.PrefixShort + argument.ShortName
            : Settings.PrefixLong + argument.Name;

        return argument.ValueType.IsArray ? new(
            argumentName,
            (argument.DefaultValue as IEnumerable)?.Cast<object>(),
            isImplicit
        ) : new ArgumentValues(
            argumentName,
            isImplicit,
    ...
    )
    {
        switch (defaultValue)
        {
            case bool defaultParsed:
            {
                ...
                {
                    if (string.Equals(
                            Settings.Localization.ValueTrue.ToString(),
                            source,
                        ...
                        ))
                    {
                        ...
                        value = true;
                                 ...
                                 Settings.Localization.ValueFalse.ToString(),
                                 source,
                                 StringComparison.OrdinalIgnoreCase
                             ))
                    {
